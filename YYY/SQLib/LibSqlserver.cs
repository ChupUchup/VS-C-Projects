...
namespace SQLib
{
    public class LibSqlserver
    {
        ...
        public LibSqlserver(string connString)
        {
            this._ConnString = connString;
            _Conn = new SqlConnection(_ConnString);
            try { _Conn.Open(); _IsConnect = true; }
            catch (SqlException ex) { _IsConnect = false; throw ex; }
            finally { _Conn.Close(); }
        }
        ...
        public DataSet Select(string query)
        {
            ...
            using (var da = new SqlDataAdapter(query, _Conn))
            {
                _Conn.Open();
                da.Fill(hsl);
                _Conn.Close();
